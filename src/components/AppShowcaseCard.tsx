  ...
  const formatHowToOwn = (text: string) => {
    ...
    return lines.map((line, index) => {
      if (line.startsWith('**') && line.endsWith('**')) {
        return (
          <h4 key={index} className="text-xl font-bold text-gray-900 dark:text-white mt-6 mb-3 flex items-center gap-2">
      ...
      if (line.match(/^\d+\./)) {
        return (
          <div key={index} className="flex items-start gap-3 mb-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-sm font-bold mt-1">
              {line.match(/^(\d+)\./)?.[1]}
            </div>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{line.replace(/^\d+\.\s*/, '')}</p>
          </div>
        );
      }
      if (line.startsWith('*')) {
        return (
          <div key={index} className="flex items-start gap-2 mb-2 ml-4">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm">{line.replace(/^\*\s*/, '')}</p>
          </div>
        );
      }
      return (
      ...
      <div className="card p-4 sm:p-6 lg:p-8 mb-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 mb-4 sm:mb-6">
            ...
            <span className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">(4.9/5 من 1,247 مراجعة)</span>
          </div>
        </div>

        {/* App Link */}
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8">
          <div className="flex items-center gap-3 mb-3 sm:mb-4">
            <Download className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">تحميل التطبيق</h3>
          </div>
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">المميزات الرئيسية</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {features.map((feature, index) => (
              <div
                key={index}
                className="flex items-start gap-3 p-3 sm:p-4 bg-white dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 hover:border-blue-300 hover:shadow-md transition-all duration-300"
              >
                <div className="w-5 h-5 sm:w-6 sm:h-6 bg-green-100 dark:bg-green-900 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                  <Check className="w-3 h-3 sm:w-4 sm:h-4 text-green-600 dark:text-green-400" />
                </div>
                <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{feature}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">كيفية الحصول على التطبيق</h3>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <button className="flex-1 px-4 py-2 sm:px-6 sm:py-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 hover:border-gray-400 transform hover:scale-105 transition-all duration-300 shadow-sm hover:shadow-md text-sm sm:text-base">
              تواصل معنا
            </button>
          </div>
        </div>
